...
async function main() {
  ...
  console.log(`\n=== All Dental-Related Slugs ===\n`);

  // Method 1: Check content_slug_ranked for slugs with "dental" in name
  ...
  const slugsByName = await queryCsv(slugRanked, `
    ...
    FROM t
    WHERE LOWER(content_slug) LIKE '%dental%'
    ORDER BY CAST(revenue AS DOUBLE) DESC
  `);

  // Method 2: Get slugs from source data that drive dental keywords
    ...
    if (sourceFiles.length > 0) {
      ...
      try {
        slugsByKeywords = await queryCsv(sourcePath, `
          ...
          FROM t
          WHERE LOWER(SERP_KEYWORD) LIKE '%dental%'
  ...
  console.log(`Found ${allSlugs.length} dental-related slugs:\n`);
  ...
  // Show all slugs
  console.log('\n📊 ALL DENTAL SLUGS (sorted by revenue):\n');
