...
import { extractKeywords, KeywordRow } from './extract_keywords';
...
async function ensureTable() {
  // No-op here; recommend running setup_pgvector.ts separately
}
...
async function main() {
  ...
  if (!input) {
    console.error('Missing --input path to angle_full.csv');
    process.exit(1);
  }

  const abs = path.resolve(input);
  let rows = await extractKeywords(abs);
  ...
  try {
    ...
    // Process in small chunks to avoid huge statements
...
if (require.main === module) {
  main().catch((err) => {
    console.error('embed_keywords failed', err);
    process.exitCode = 1;
  });
}
