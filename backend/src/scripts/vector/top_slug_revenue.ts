...
async function main() {
  ...
  const slugRanked = path.resolve(`./runs/system1/${runDate}/content_slug_ranked.csv`);
  ...
  console.log(JSON.stringify(topSlugs.map(r => Object.fromEntries(Object.entries(r).map(([k,v]) => [k, typeof v === 'bigint' ? Number(v) : v]))), null, 2));
}
