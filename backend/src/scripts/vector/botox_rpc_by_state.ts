...
async function main() {
  ...
  const botoxStates = await queryCsv(stateCsv, `
    SELECT 
      state,
    ...
    ORDER BY rpc DESC NULLS LAST
  `);
  
  console.log(JSON.stringify(botoxStates.map(r => Object.fromEntries(Object.entries(r).map(([k,v]) => [k, typeof v === 'bigint' ? Number(v) : v]))), null, 2));
}
