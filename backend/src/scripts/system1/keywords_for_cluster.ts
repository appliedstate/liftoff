...
async function main() {
  ...
  // Query keywords directly from CSV
  console.log('Querying CSV for keywords...\n');
  ...
  // Calculate revenue per root phrase
  ...
  // Find root phrases that make up 80% of revenue
  let cumulativeRevenue = 0;
  const topRootPhrases: typeof rootPhrases = [];
  
  for (const rootPhrase of rootPhrases) {
    cumulativeRevenue += rootPhrase.revenue;
    topRootPhrases.push(rootPhrase);
    if (cumulativeRevenue >= targetRevenue) {
      break;
    }
  }
  
  console.log(`\n📊 ROOT PHRASES PRODUCING 80% OF REVENUE:\n`);
  console.log(`Found ${topRootPhrases.length} root phrases (out of ${rootPhrases.length} total)\n`);
  
  for (let i = 0; i < topRootPhrases.length; i++) {
    const rp = topRootPhrases[i];
    const rpc = rp.clicks > 0 ? rp.revenue / rp.clicks : 0;
    const rps = rp.searches > 0 ? rp.revenue / rp.searches : 0;
    
    console.log(`${(i + 1).toString().padStart(2)}. "${rp.root}"`);
  ...
  console.log(`Total Keywords Covered: ${topRootPhrases.reduce((sum, rp) => sum + rp.variants, 0)}`);
  
  // Also show top individual keywords (not grouped)
  console.log(`\n\n🔍 TOP 20 INDIVIDUAL KEYWORDS (not grouped):\n`);
  keywordList.slice(0, 20).forEach((kw, idx) => {
    const rpc = kw.clicks > 0 ? kw.revenue / kw.clicks : 0;
    console.log(`${(idx + 1).toString().padStart(2)}. "${kw.keyword}"`);
    console.log(`    Revenue: $${kw.revenue.toFixed(2)} | Clicks: ${kw.clicks.toFixed(0)} | RPC: $${rpc.toFixed(4)}`);
  });
}

main().catch((err) => {
  console.error('keywords_for_cluster failed', err);
  process.exit(1);
});
