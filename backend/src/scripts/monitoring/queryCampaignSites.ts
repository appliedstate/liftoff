  ...
  try {
    ...
    if (byAccount.length > 0) {
      ...
      for (const row of byAccount) {
        ...
        console.log(`│ ${account} │ ${campaigns.toString().padStart(10)} │ ${sites.toString().padStart(12)} │ $${revenue.toFixed(2).padStart(12)} │ ${sessions.toFixed(0).padStart(12)} │ $${spend.toFixed(2).padStart(12)} │ ${roasStr.padStart(12)} │`);
      }
      
      const totalRoas = totalSpend > 0 ? totalRevenue / totalSpend : 0;
      const totalRoasStr = totalSpend > 0 ? `${totalRoas.toFixed(2)}x` : 'N/A';
      console.log('├─────────────────────┼────────────┼──────────────┼──────────────┼──────────────┼──────────────┼──────────────┤');
      console.log(`│ TOTAL               │ ${totalCampaigns.toString().padStart(10)} │              │ $${totalRevenue.toFixed(2).padStart(12)} │ ${totalSessions.toFixed(0).padStart(12)} │ $${totalSpend.toFixed(2).padStart(12)} │ ${totalRoasStr.padStart(12)} │`);
      console.log('└─────────────────────┴────────────┴──────────────┴──────────────┴──────────────┴──────────────┴──────────────┘');
    }
    ...
    console.log('┌──────────────┬──────────────────────────────────────────┬──────────────┬─────────────────────┬─────────────────────┬──────────────┬─────────────┬──────────────┐');
    console.log('│ Campaign ID  │ Campaign Name                             │ Media Source │ RSOC Site           │ S1 Google Account   │ Revenue      │ Sessions    │ ROAS         │');
    console.log('├──────────────┼──────────────────────────────────────────┼──────────────┼─────────────────────┼─────────────────────┼──────────────┼─────────────┼──────────────┤');
    ...
    for (const row of campaigns) {
      ...
      console.log(`│ ${campaignId} │ ${campaignName} │ ${mediaSource} │ ${rsocSite} │ ${s1Account} │ $${revenue.toFixed(2).padStart(11)} │ ${sessions.toFixed(0).padStart(11)} │ ${roasStr.padStart(12)} │`);
    }
    ...
    if (siteMapping.length > 0) {
      ...
      console.log('┌─────────────────────┬─────────────────────┬────────────┬──────────────┬──────────────┐');
      console.log('│ RSOC Site          │ S1 Google Account   │ Campaigns  │ Revenue      │ RPC          │');
      console.log('├─────────────────────┼─────────────────────┼────────────┼──────────────┼──────────────┤');
      ...
      for (const row of siteMapping) {
        ...
        // Get sessions for this site to calculate RPC
        const siteStats = await allRows(
          conn,
          `SELECT SUM(sessions) as total_sessions
          FROM campaign_index
          WHERE ${whereClause}
            AND rsoc_site = '${row.rsoc_site}'
            AND s1_google_account = '${row.s1_google_account || ''}'`
        );
        const sessions = Number(siteStats[0]?.total_sessions || 0);
        const rpc = sessions > 0 ? revenue / sessions : 0;
        const rpcStr = sessions > 0 ? `$${rpc.toFixed(2)}` : 'N/A';
        
        console.log(`│ ${site} │ ${account} │ ${campaigns.toString().padStart(10)} │ $${revenue.toFixed(2).padStart(12)} │ ${rpcStr.padStart(12)} │`);
      }
