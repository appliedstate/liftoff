...
router.get('/reconciled', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.reconciled error', err);
...
router.post('/simulate', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.simulate error', err);
...
router.get('/summary', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.summary error', err);
...
router.get('/state', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.state error', err);
...
router.post('/learn', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.learn error', err);
    return res.status(500).json({ code: 'internal_error', message: 'Learn failed' });
  }
});


// Execute decisions via Terminal
router.post('/execute', authenticateUser, async (req, res) => {
  try {
    ...
    const { terminalClient, generateIdempotencyKey } = await import('../lib/terminalClient');
...
router.post('/applied', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.applied error', err);
...
router.post('/suggest', authenticateUser, async (req, res) => {
  try {
    ...
    const summary = [
      `Decision Engine suggestions for ${date} (${level})`,
      `Snapshot: ${snap}`,
    ...
    console.error('decision-engine.suggest error', err);
...
router.post('/copilot/suggest', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.copilot.suggest error', err);
...
router.get('/policy-config', authenticateUser, async (req, res) => {
  try {
    ...
    console.error('decision-engine.policy-config error', err);
